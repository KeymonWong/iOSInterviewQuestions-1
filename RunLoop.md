...
RunLoop 顾名思义，就是一个循环，线程创建并运行事件处理程序来响应输入事件。

RunLoop的管理并不是全自动的，需要程序设计人员设计线程代码在适当的时候启动RunLoop并正确的响应输入事件。Cocoa 和 CoreFundation 都提供了RunLoop对象方便配置和管理线程的RunLoop，除了主线程(Main thread)拥有与之对应的RunLoop对象(自行创建并运行)，自建的线程需要手动运行RunLoop，在run loop中，使用run loop对象来运行事件处理代码：响应接收到的事件，启动已经安装的处理程序。Run loop处理的输入事件有两种不同的来源：输入源（input source）和定时源（timer source）。输入源传递异步消息，通常来自于其他线程或者程序。定时源则传递同步消息，在特定时间或者一定的时间间隔发生。两种源的处理都使用程序的某一特定处理路径。

本文将从 CFRunLoop 的源码入手，介绍 RunLoop 的概念以及底层实现原理。
介绍如何实现：
...
CFRunLoopRef 的代码是[开源](https://opensource.apple.com/source/CF/CF-855.17/CFRunLoop.c)的，你可以在这里 http://opensource.apple.com/tarballs/CF/ 下载到整个 CoreFoundation 的源码来查看。
...
![](https://ws2.sinaimg.cn/large/006tKfTcly1fpyev8l28bj30pa0ju75l.jpg)
...
关于Mach的历史可以看看这篇很有趣的文章：[Mac OS X 背后的故事（三）Mach 之父 Avie Tevanian](http://gitbook.cn/8121/)。
...
当一个硬件事件(触摸/锁屏/摇晃等)发生后，首先由 IOKit.framework 生成一个 IOHIDEvent 事件并由 SpringBoard 接收。这个过程的详细情况可以[参考](http://iphonedevwiki.net/index.php/IOHIDFamily)这里。SpringBoard 只接收按键(锁屏/静音等)，触摸，加速，接近传感器等几种 Event，随后用 mach port 转发给需要的App进程。随后苹果注册的那个 Source1 就会触发回调，并调用 _UIApplicationHandleEventQueue() 进行应用内部的分发。
...
具体的代码可以看这里：[_ASAsyncTransactionGroup](https://github.com/facebookarchive/AsyncDisplayKit/blob/master/AsyncDisplayKit%2FDetails%2FTransactions%2F_ASAsyncTransactionGroup.m)。
...
[参考文档](https://blog.ibireme.com/2015/05/18/runloop/)
